...
switch($switchc) {
			...
			if ($jakdb->has("sessions", ["uniqueid" => $message])) {
				...
				if (isset($result) && !empty($result)) foreach ($result as $row) {
					...
					if (JAK_base::jakAvailableHours($row["hours_array"], date('Y-m-d H:i:s')) || $row["alwaysnot"] == 1) {
						...
						$url = str_replace('include/', '', JAK_rewrite::jakParseurl(JAK_OPERATOR_LOC, 'live', $cid));
				...
				$cudetails = jak_string_encrypt_decrypt($restorechat["id"].":#:".$restorechat["uniqueid"].":#:".$restorechat["userid"].":#:".$restorechat["name"].":#:".$restorechat["email"].":#:".$restorechat["phone"].":#:".$restorechat["usr_avatar"].":#:".$restorechat["lang"]);
				...
				if ($cid) {
					...
					$cudetails = jak_string_encrypt_decrypt($cid.":#:".$unique_restore.":#:".$userid.":#:".$clientname.":#:".$clientemail.":#:".$uphone.":#:".$avatar.":#:".$lang);
					...
					$chatgpt = false;
					if (JAK_OPENAI_CHATGPT && !empty(JAK_OPENAI_APIKEY) && $jakwidget['chatgpt']) {

						// Let's make the call
						$botanswer = jak_chatgpt_chat($message, "", $jakwidget['chatgpt_helpful'], $jakwidget["id"], $lang, $dep_direct);
					...
					if (!empty($JAK_BOT_ANSWER) && !$chatgpt) {
	...
	case 'start':
		...
		if (JAK_EMAIL_BLOCK && ((isset($widgetsettings['start_email_required']) && $widgetsettings['start_email_required'] == "Yes") || !empty($jkp['start_email']))) {
			...
			if (in_array($jkp['start_email'], $blockede) || in_array(strrchr($jkp['start_email'], "@"), $blockede)) {
			...
			if (isset($cudetails[0]) && is_numeric($cudetails[0]) && $jakdb->has("sessions", ["session" => $_POST['rlbid']])) {
				...
				if (isset($row) && !empty($row)) {
					...
					if (isset($message) && !empty($message) && $row['hide'] <= 1) {
						...
						// We have ChatGTP build in, let's check if we run it
						$chatgpt = false;
						if (empty($row["operator"]) && JAK_OPENAI_CHATGPT && !empty(JAK_OPENAI_APIKEY) && $jakwidget['chatgpt']) {
						...
						if (empty($row["operator"]) && !empty($JAK_BOT_ANSWER) && !$chatgpt) {
						...
						if ($row['pusho'] && (time() - $row['statuso']) > JAK_PUSH_REMINDER) {
							...
							if ($pushorow["push_notifications"]) {
								...
								$url = str_replace('include/', '', JAK_rewrite::jakParseurl(JAK_OPERATOR_LOC, 'live', $row['convid']));
						...
						die(json_encode(array('status' => true, "html" => $chatmsg, 'lastid' => $lastid, 'lastmsg' => $message, "placeholder" => $jkl["g6"], "chatgpt" => $chatgpt)));
					...
					// Chat is hidden, no more messages and end the session
						...
						die(json_encode(array('status' => true, "html" => $chatmsg, 'lastid' => $lastid, "placeholder" => $jkl["g6"])));
			...
			if (isset($cudetails[0]) && is_numeric($cudetails[0]) && $jakdb->has("sessions", ["session" => $_POST['rlbid']])) {
				...
				if (isset($row) && !empty($row)) {
					...
					if (empty($row["operator"]) && JAK_OPENAI_CHATGPT && !empty(JAK_OPENAI_APIKEY) && $jakwidget['chatgpt']) {
						...
						// We need one more query to get the details
						$widgetdet = $jakdb->get("sessions", ["widgetid", "lang"], ["session" => $_POST['rlbid']]);

						// Ok, before we run chatgpt we browse through the already stored answers
						// $answer = $jakdb->get("bot_chatgpt", "answer", "MATCH" => ["columns" => ["question"], "keyword" => $message, "mode" => "boolean"],"widgetid" = $widgetdet["widgetid"], "ORDER" => ["updated" => "DESC"], "LIMIT" => 1);

						// Advanced Method to find the correct answer
						$sth = $jakdb->pdo->prepare('SELECT answer, MATCH (question, answer) AGAINST (:message IN BOOLEAN MODE) AS relevance FROM '.JAKDB_PREFIX.'bot_chatgpt WHERE widgetids = :widgetid AND active = 1 AND MATCH (question, answer) AGAINST (:message2 IN BOOLEAN MODE) ORDER BY MATCH (question, answer) AGAINST (:message3 IN BOOLEAN MODE) DESC LIMIT 1');

						$sth->bindParam(':widgetid', $widgetdet['widgetid'], PDO::PARAM_INT);
						$sth->bindParam(':message', $message, PDO::PARAM_STR, 255);
						$sth->bindParam(':message2', $message, PDO::PARAM_STR, 255);
						$sth->bindParam(':message3', $message, PDO::PARAM_STR, 255);
						...
						if (isset($answer) && !empty($answer)) {
							...
							// Place holder converters, now possible
							$phold = array("%client%","%email%");
							$replace   = array($cudetails[3], JAK_EMAIL);
							$answer = str_replace($phold, $replace, $answer);
						...
						if (empty($answer)) {
							...
							// We need the previous messages from chatgpt
							$gptmsgs = $jakdb->select("transcript", ["message"], ["AND" => ["convid" => $row['convid'], "class" => "bot"]]);

							// Let's make the call
							$answer = jak_chatgpt_chat($message, $gptmsgs, $jakwidget['chatgpt_helpful'], $widgetdet["widgetid"], $widgetdet["lang"], $row["depid"]);
					...
					// We will need the template
					$lcdnm = true;
					include_once APP_PATH.'lctemplate/'.$jakwidget['template'].'/tplblocks/chat/'.$jakwidget["chat_tpl"];

					// Now attach the bot message
					if (!empty($botdisp) && $chatgpt) {

						$chatmsg = $wtplsett["chatbotinsert"];

						die(json_encode(array('status' => true, "html" => $chatmsg)));
					} else {
						die(json_encode(array('status' => false, 'error' => stripcslashes($jkl['e2']))));
					}
				...
				if (isset($row) && !empty($row)) {
					...
					if (empty($row['operator'])) {
						...
						if (!empty($LC_ANSWERS) && is_array($LC_ANSWERS)) foreach ($LC_ANSWERS as $v) {
							...
							if ($v["msgtype"] == 1 && $v["lang"] == $lang && ($v["department"] == 0 || $v["department"] == $row["depid"]) && $row['initiated'] < ($currentime - $v["fireup"])) {

								$standardmsg_exist = $jakdb->has("transcript", ["AND" => ["convid" => $row['convid'], "standardmsg" => $v["id"]]]);

								if (!$standardmsg_exist) {
					...
					$newcudetails = "";
					if ($row['datac']) {
						$rowc = $jakdb->get("sessions", ["lang", "name", "email", "phone"], ["id" => $row['convid']]);
						...
						$newcudetails = jak_string_encrypt_decrypt($cudetails[0].":#:".$cudetails[1].":#:".$cudetails[2].":#:".$rowc['name'].":#:".$newemail.":#:".$rowc['phone'].":#:".$cudetails[6].":#:".$rowc['lang']);
	...
	case 'aitranslate':
		...
		// Let's explode the string (0 = convid, 1 = uniqueid, 2 = userid, 3 = name, 4 = email, 5 = phone, 6 = avatar, 7 = lang)
		$cudetails = explode(":#:", $cudetails);

		// Let's filter some inputs
		$cid = filter_var($cudetails[0], FILTER_SANITIZE_NUMBER_INT);
		$caid = filter_var($_GET['msgid'], FILTER_SANITIZE_NUMBER_INT);

		// Get the language
		$chatlang = $jakdb->get("sessions", ["lang"], ["id" => $cid]);

		// Get the message itself
		$chatmsg = $jakdb->get("transcript", ["message"], ["AND" => ["id" => $caid, "convid" => $cid]]);

		// We do need to check the translation table
		$transl = $jakdb->get("chat_ai_translations", ["original_lang", "translated_lang", "lang"], ["AND" => ["chatid" => $cid, "answerid" => $caid]]);

		// We check if the text is the same and the translation has already been done
		if (isset($transl["lang"]) && $transl["lang"] == $cudetails[7]) {
		    ...
		    $translation_msg[] = [
		        "role" => "user",
		        "content" => "Please translate this text into following language with country code '".$cudetails[7]."': ".$chatmsg["message"]
		    ];
		...
		die(json_encode(array('status' => true, "translation" => $translation)));
	...
	case 'loadprofile':
		...
		if (isset($_POST['customer']) && !empty($_POST['customer'])) {
			...
			if (isset($cudetails[0]) && is_numeric($cudetails[0])) {
				...
				// Go back to the chat
				die(json_encode(array('status' => true, 'avatar' => $cudetails[6], 'name' => $cudetails[3], 'email' => $cudetails[4], 'phone' => $cudetails[5], 'lang' => $cudetails[7])));
		...
		if (count($errors) > 0) {
			...
			if (isset($_POST['customer']) && !empty($_POST['customer'])) {
				...
				if (isset($cudetails[0]) && is_numeric($cudetails[0])) {
					...
					$jakdb->update("sessions", ["lang" => $jkp["profile_language"], "name" => $newname, "email" => $jkp['email'], "phone" => $jkp['phone'], "usr_avatar" => $avatar], ["id" => $cudetails[0]]);
					...
					// Get the details encrypted
					$cudetails = jak_string_encrypt_decrypt($cudetails[0].":#:".$cudetails[1].":#:".$cudetails[2].":#:".$newname.":#:".$jkp['email'].":#:".$jkp['phone'].":#:".$avatar.":#:".$jkp["profile_language"]);
		...
		if (count($errors) > 0) {
			...
			if (isset($_POST['customer']) && !empty($_POST['customer'])) {
				...
				if (isset($cudetails[0]) && is_numeric($cudetails[0])) {
					...
					// We prepare the form if needed
					$formrow = $jakdb->get("sessions", ["department"], ["id" => $cudetails[0]]);
					...
					$op_email = JAK_EMAIL;
					if (is_numeric($formrow["department"]) && $formrow["department"] != 0) {
						
						if (isset($LC_DEPARTMENTS)) foreach ($LC_DEPARTMENTS as $d) {
						    if (in_array($formrow["department"], $d)) {
					...
					// Now get the support time and update the stats table
					$statrow = $jakdb->get("sessions", ["name", "initiated", "ended", "operatorid"], ["id" => $cudetails[0]]);
				
					$total_supporttime = $statrow['ended'] - $statrow['initiated'];

					if (!JAK_CRATING) $jkp["rating"] = 0;

					$jakdb->insert("user_stats", [ 
						"userid" => $statrow["operatorid"],
						"vote" => $jkp["rating"],
						"name" => $statrow['name'],
						"email" => $email,
						"comment" => $message,
